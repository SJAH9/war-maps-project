(() => {
  ...
  if(!war||!health||!births||!population||!window.THREE){
    const missing=[!war&&'conflict data',!health&&'mortality and fertility data',!births&&'birth-rate data',!population&&'population data',!window.THREE&&'3D renderer'].filter(Boolean);
    document.addEventListener('DOMContentLoaded',()=>{const container=document.querySelector('#mortality-map');if(container)container.innerHTML=`<p class="boundary-note mortality-error">The Life and Death field could not start because ${missing.join(', ')} did not load. Reload the page or check the disclosed asset paths.</p>`;});
    return;
  }
  ...
  const state={active:new Set(['conflict','population','mortality','fertility','birth']),healthYear:2023,start:'',end:'',geometry:null,birthByMap:new Map(),populationByMap:new Map(),globalPopulation:new Map(population.global||[]),observations:new Map(),selected:'',countryMeshes:[],blocks:[],barGroup:null,metricRailGroup:null,metricRailEpoch:0,metricRailReady:false,metricRailOrder:[...RAIL_METRICS],playing:false,playbackStarted:false,playbackTimer:null,scene:null,camera:null,renderer:null,controls:null,raycaster:new THREE.Raycaster(),pointer:new THREE.Vector2()};
  ...
  function aggregate(){
    ...
    war.events.filter(inRange).forEach(event=>{const item=ensure(mapName(event.country||'Unspecified territory'));item.conflict.events++;});
  ...
  function addStarrySky(scene){
    let seed=14031945;const random=()=>((seed=(seed*1664525+1013904223)>>>0)/4294967296),stars=[];for(let index=0;index<1050;index++){const azimuth=random()*Math.PI*2,elevation=-.28+Math.pow(random(),1.25)*1.15,radius=720+random()*120,horizontal=Math.cos(elevation)*radius;stars.push(Math.cos(azimuth)*horizontal,Math.sin(elevation)*radius-60,Math.sin(azimuth)*horizontal);}const starGeometry=new THREE.BufferGeometry();starGeometry.setAttribute('position',new THREE.Float32BufferAttribute(stars,3));const starField=new THREE.Points(starGeometry,new THREE.PointsMaterial({color:'#e4edf5',size:1.8,sizeAttenuation:true,transparent:true,opacity:dark()?.78:.46,depthWrite:false,fog:false}));starField.renderOrder=-19;scene.add(starField);
  }
  ...
  function buildScene(){
    const container=$('#mortality-map'),scene=new THREE.Scene(),background=dark()?'#050b13':'#596a73';scene.background=new THREE.Color(background);scene.fog=new THREE.FogExp2(background,.0018);state.scene=scene;
    const camera=new THREE.PerspectiveCamera(34,1,.1,1400);camera.position.set(185,190,285);state.camera=camera;
    const renderer=new THREE.WebGLRenderer({antialias:false,preserveDrawingBuffer:false});renderer.setPixelRatio(Math.min(devicePixelRatio,1.25));renderer.outputEncoding=THREE.sRGBEncoding;container.replaceChildren(renderer.domElement);state.renderer=renderer;
    const controls=new THREE.OrbitControls(camera,renderer.domElement);controls.target.set(0,MAP_Y+8,-20);controls.enableDamping=true;controls.dampingFactor=.06;controls.enablePan=false;controls.minDistance=150;controls.maxDistance=900;controls.minPolarAngle=.52;controls.maxPolarAngle=1.22;controls.update();const azimuth=controls.getAzimuthalAngle();controls.minAzimuthAngle=azimuth-Math.PI/2;controls.maxAzimuthAngle=azimuth+Math.PI/2;state.controls=controls;
    scene.add(new THREE.HemisphereLight(dark()?'#c5ddd9':'#ffffff',dark()?'#0b1518':'#25383c',1.25));const key=new THREE.DirectionalLight('#ffe0a1',1.35);key.position.set(-90,180,80);scene.add(key);addStarrySky(scene);addIVMField(scene);
  ...
  function valueLabel(item,metric){const value=valueFor(item,metric);if(metric==='conflict')return `${format(value)} candidate observations`;if(metric==='population')return item?.population?`${format(value)} people`:'No observation';if(metric==='mortality')return item?.mortality?`${format(value,1)} deaths per 100,000`:'No observation';if(metric==='fertility')return item?.fertility?`${format(value,2)} births per woman`:'No observation';return item?.birth?`${format(value,1)} live births per 1,000`:'No observation';}
  function renderInspector(country=''){
    ...
    const metrics=item?[
      ['Conflict observations',valueLabel(item,'conflict')],['Population',valueLabel(item,'population')],
      ...
      ['Conflict observations',format(totals.conflict)],['Population observations',totals.population],
    ...
    $('#mortality-boundary').textContent=`Conflict stacks count UCDP candidate observations from ${state.start} through ${state.end}; they do not sum casualty estimates because candidate records may overlap. Mortality and total fertility use IHME GBD 2023 results when available (1980-2023). Crude birth rate uses World Bank WDI / UN observations for 1960-2024; total population uses the corresponding 1960-2025 WDI series. Every metric is normalized independently because counts and rates are not commensurate. Missing source years remain missing. Event territory is not victim nationality.`;
    updateSelection();
  }
  ...
  function syncHealthYear(value,render=true){
    const year=$('#health-year'),next=Math.max(Number(year.min),Math.min(Number(year.max),Number(value)));state.healthYear=next;year.value=next;$('#health-year-output').textContent=next;$('#playback-date').textContent=next;if(render)updateColumns();
  }
  function setPlayback(playing){
    state.playing=playing;clearTimeout(state.playbackTimer);const button=$('#playback-toggle');button.setAttribute('aria-pressed',String(playing));button.textContent=playing?'Ⅱ':'▶';button.title=playing?'Pause measure-year playback':'Play through measure years';button.setAttribute('aria-label',button.title);if(playing)queuePlayback();
  }
  function queuePlayback(){
    clearTimeout(state.playbackTimer);if(!state.playing)return;const speed=Number($('#playback-speed').value)||1;state.playbackTimer=setTimeout(()=>{const maximum=Number($('#health-year').max);if(state.healthYear>=maximum){state.playbackStarted=false;setPlayback(false);return;}syncHealthYear(state.healthYear+1);queuePlayback();},1000/speed);
  }
  function bindControls(){
    const dates=war.events.flatMap(event=>[event.date_start,event.date_end||event.date_start]).filter(Boolean).sort();state.start=dates[0];state.end=dates.at(-1);$('#mortality-start').min=state.start;$('#mortality-start').max=state.end;$('#mortality-start').value=state.start;$('#mortality-end').min=state.start;$('#mortality-end').max=state.end;$('#mortality-end').value=state.end;
    const metricInputs=[...document.querySelectorAll('[data-metric]')];state.active=new Set(metricInputs.filter(input=>input.checked).map(input=>input.dataset.metric));state.metricRailOrder=RAIL_METRICS.filter(metric=>state.active.has(metric));metricInputs.forEach(input=>input.addEventListener('change',()=>{const metric=input.dataset.metric;if(input.checked){state.active.add(metric);if(RAIL_METRICS.includes(metric)&&!state.metricRailOrder.includes(metric))state.metricRailOrder.push(metric);}else{state.active.delete(metric);state.metricRailOrder=state.metricRailOrder.filter(item=>item!==metric);}updateColumns()}));
    const year=$('#health-year'),speed=$('#playback-speed');syncHealthYear(year.value,false);let yearTimer;year.addEventListener('input',event=>{state.playbackStarted=true;syncHealthYear(event.target.value,false);clearTimeout(yearTimer);yearTimer=setTimeout(updateColumns,120)});year.addEventListener('change',()=>{clearTimeout(yearTimer);updateColumns()});
    const showSpeed=()=>$('#playback-speed-output').textContent=`${Number(speed.value).toLocaleString(undefined,{maximumFractionDigits:2})}×`;showSpeed();speed.addEventListener('input',()=>{showSpeed();if(state.playing)queuePlayback()});$('#playback-toggle').addEventListener('click',()=>{if(!state.playing&&(!state.playbackStarted||state.healthYear>=Number(year.max))){syncHealthYear(year.min);state.playbackStarted=true;}setPlayback(!state.playing)});document.addEventListener('visibilitychange',()=>{if(document.hidden&&state.playing)setPlayback(false)});
    $('#mortality-start').addEventListener('change',event=>{state.start=event.target.value;if(state.start>state.end){state.end=state.start;$('#mortality-end').value=state.end}updateColumns()});$('#mortality-end').addEventListener('change',event=>{state.end=event.target.value;if(state.end<state.start){state.start=state.end;$('#mortality-start').value=state.start}updateColumns()});$('#mortality-reset').addEventListener('click',resetView);
