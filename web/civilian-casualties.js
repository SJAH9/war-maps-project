(() => {
  ...
  const state = {search:'', selected:null, nation:null, reverseCountries:new Set(), visible:[], worldVisible:[], priceScale:{min:0,max:1}, healthScale:{mortality:{min:0,max:1},lifeExpectancy:{min:0,max:1}}, activeHealth:new Set(['mortality','lifeExpectancy']), startYear:defaultStart, endYear:defaultEnd, playTimer:null, playIndex:0, scene:null, renderer:null, camera:null, controls:null, plateRoot:null, worldLandGroup:null, reverseGroup:null, towerGroup:null, reverseTowerGroup:null, worldMeshes:[], reverseMeshes:[], towers:[], worldTowers:[], reverseTowers:[], countryAnchors:[], countryAnchorGroup:null, hoveredTower:null, raycaster:null, pointer:null, flipTarget:0, flipping:false, pointerDown:null};
  ...
  function init() {
    ...
    $('#civ-reset').addEventListener('click',()=>{stopPlay();if(state.nation)flipToWorld();state.search='';state.selected=null;state.activeHealth=new Set(['mortality','lifeExpectancy']);document.querySelectorAll('[data-health-layer]').forEach(input=>{input.checked=true;});state.startYear=defaultStart;state.endYear=defaultEnd;$('#civ-start').value=String(defaultStart);$('#civ-end').value=String(defaultEnd);$('#civ-search').value='';if(state.camera){state.camera.position.set(185,190,285);state.controls.target.set(0,MAP_Y+8,-20);state.controls.update();}filterRows();});
