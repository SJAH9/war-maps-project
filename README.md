...
The top-level **Graph** view joins states only when UCDP records them participating on the same side during at least one shared year or candidate event. Edge width represents shared years, and node size represents same-side degree. Selecting a state identifies its partners, historic opposing states, and opponents reachable through a mutual same-side partner. Opposing participation is disclosed in the inspector but never introduced as a graph edge.
