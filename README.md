...
The top-level **Graph** view exposes the connection rule rather than hiding it. The observed layer joins states only when UCDP records them participating on the same side during at least one shared year or candidate event. A separate organization layer can draw sourced co-membership edges for the United Nations, NATO, BRICS, the European Union, or ASEAN, and can combine those edges with observed participation. The World Economic Forum is retained as a typed non-state membership source; its company and stakeholder relationships are not converted into state membership. Edge width represents shared years for observed edges, while organization edges retain their organization semantics. Selecting a state identifies its partners, historic opposing states, and opponents reachable through a mutual same-side partner. Opposing participation is disclosed in the inspector but never introduced as a graph edge. The topology comparison control can generate deterministic Barabási–Albert, Erdős–Rényi, or Watts–Strogatz synthetic graphs, which are explicitly marked as modeled comparisons and never treated as evidence.
