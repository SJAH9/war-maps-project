...
Four large metric rows occupy the same 3D plane north of the map. They pair color-matched block lettering with exact global population, summed visible conflict fatalities, and clearly labelled displayed-country means for mortality and fertility. Dotted leaders connect left-aligned metric names to right-aligned values. Orbit controls preserve a readable oblique orientation: panning is disabled and azimuth, polar angle, and zoom are bounded so the in-scene type cannot be rotated upside down or behind the map.
