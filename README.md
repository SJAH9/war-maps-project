...
The top-level **Life and Death** view floats Natural Earth country geometry above a thin golden isotropic-vector-matrix lattice and places independently toggled stacks at each state: UCDP candidate-event fatalities, IHME GBD 2023 age-standardized all-cause mortality, a period total fertility rate derived from IHME age-specific fertility rates, and World Bank WDI crude birth rate sourced primarily from UN World Population Prospects. Conflict is encoded from black to red, mortality in navy blue, total fertility in deep violet, and crude birth rate from deep olive to neon green. A single fixed phi-accelerated block form keeps low observations near the map while separating the upper tail. Each metric remains independently normalized because the values do not share a unit; actual values and units remain in the inspector. The 1960-2024 measure-year control exposes birth-rate coverage while preserving missing IHME values outside 1980-2023.
