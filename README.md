...
The top-level **Life and Death** view floats Natural Earth country geometry above a thin golden isotropic-vector-matrix lattice beneath a starry near-dawn sky. The sky is a solid pre-dawn field without a gradient or finite dome; sharp spatial stars establish depth while the IVM lattice alone generates the horizon. Independently toggled stacks at each state show UCDP candidate-event fatalities, World Bank / UN total population, IHME GBD 2023 age-standardized all-cause mortality, period total fertility, and World Bank / UN crude birth rate. Conflict is encoded from black to red, population in gold, mortality in navy blue, total fertility in deep violet, and crude birth rate from deep olive to neon green. Each metric remains independently normalized because the values do not share a unit; actual values and units remain in the inspector. The 1960-2025 measure-year control preserves missing observations outside each source's temporal boundary.

Four large metric rows rise beyond the north edge of the map. On its first draw, the rail eases through a 90-degree rotation toward the map and finishes as an upright stack on the same vertical axis as the data columns. Color-matched metric names appear alone at left, dotted leaders cross the field, and numeric values appear alone at right. Disabling a metric removes its row and slides every row above it down to close the gap; re-enabling a metric always adds its row to the top. The values are exact global population, summed visible conflict fatalities, and displayed-country means for mortality and fertility; the method page supplies their units and aggregation context. Orbit controls allow a 180-degree front-facing horizontal arc and a broad 150–900-unit zoom range while preserving a readable oblique orientation: panning is disabled and polar angle remains bounded so the in-scene type cannot be rotated upside down or viewed from behind.
