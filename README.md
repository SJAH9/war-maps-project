...
The **3D Network Browser** extends that connection-following method across the complete loaded archive. Its lazy index connects nations, conflicts, candidate events, named human references, organizations and conflict actors, and recorded places. Clicking a node moves it to the center and loads every directly connected node plus the lateral relationships among that neighborhood. Hover labels and the persistent color key identify node types, while the right-hand Connections register provides a text route through the same graph. Search can begin a path anywhere in the index. Optimization landscapes change only spatial emphasis; the underlying source relationships remain fixed.
