...
Four large metric rows rise beyond the north edge of the map. On each redraw, the rail eases through a 90-degree rotation toward the map and finishes as an upright stack on the same vertical axis as the data columns. Color-matched metric names appear alone at left, dotted leaders cross the field, and numeric values appear alone at right. Each row follows its metric toggle and the remaining rows close up when one is hidden. The values are exact global population, summed visible conflict fatalities, and displayed-country means for mortality and fertility; the method page supplies their units and aggregation context. Orbit controls preserve a readable oblique orientation: panning is disabled and azimuth, polar angle, and zoom are bounded so the in-scene type cannot be rotated upside down or behind the map.
