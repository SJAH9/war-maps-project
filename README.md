...
The **Relationship Browser** exposes the connection rule rather than hiding it. The observed layer joins states only when UCDP records them participating on the same side during at least one shared year or candidate event. A separate organization layer can draw sourced co-membership edges for the United Nations, NATO, BRICS, the European Union, or ASEAN, and can combine those edges with observed participation. The World Economic Forum is retained as a typed non-state membership source; its company and stakeholder relationships are not converted into state membership. Edge width represents shared years for observed edges, while organization edges retain their organization semantics. Selecting any displayed node makes it central and redraws its complete immediate neighborhood; selecting a neighbor continues the path. The equilibrium, connection-shell, opposing-field, bridge, influence-hierarchy, and temporal optimization landscapes rearrange the same observed edges and never fabricate relationships.
