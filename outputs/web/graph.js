(()=>{
  ...
  const state={minYears:1,throughYear:2025,view:'network',relationship:'combined',organizations:[],includeUngrouped:false,topology:'equilibrium',selected:'',nodes:new Map(),links:[],model:null,adjacency:new Map(),opponents:new Map(),bridges:new Map(),svgScene:null,omittedNodes:0};
  const colors={base:'#7b8051',isolated:'#4e5145',selected:'#ffd500',ally:'#8b989b',bridge:'#ff8a1f',opponent:'#8f2f27',dim:'#34372f'};
  const relationshipLabel=()=>state.relationship==='participation'?'nation–conflict participation':state.relationship==='observed'?'same-side participation':state.relationship==='organization'?'organization membership':'nation, organization, and conflict relationships';
  const linkDescription=link=>link.kind==='organization'?`${link.organizations?.join(', ')||'Organization'} membership`:link.kind==='conflict'?`${link.years} recorded conflict-year${link.years===1?'':'s'}`:`${link.years} shared years`;
  ...
  function updatePageDescription(){
    const selected=selectedOrganizations(),names=selected.map(item=>item.name).join(', '),wef=selected.some(item=>item.id==='wef'),title=wef&&selected.length===1?'WEF partner home-nation relationships':selected.length?`Membership relationships: ${names}`:state.relationship==='organization'?'Organization membership relationships':'Relationship Browser';
  ...
  function buildModel(){
    ...
    const selected=selectedOrganizations(),selectedMemberNames=selected.length&&!state.includeUngrouped?new Set(selected.flatMap(organizationMembers)):null,includeOrganizationNodes=selected.length&&['organization','combined'].includes(state.relationship);
    const nationNodes=data.nations.filter(profile=>!selectedMemberNames||selectedMemberNames.has(profile.country)).map(profile=>({
      id:profile.country,label:profile.country,profile,entityKind:'nation',
    ...
    const nationIds=new Set(nationNodes.map(node=>node.id));
    const edges=new Map();
    data.nations.forEach(profile=>normalizedRelations(profile,'same_side_partners').forEach(relation=>{
      ...
      if(!nationIds.has(profile.country)||!nationIds.has(relation.country)||years<state.minYears||Number(relation.first_year||0)>state.throughYear||relation.country===profile.country)return;
    ...
    (data.conflict_years||[]).filter(row=>Number(row.year)<=state.throughYear).forEach(row=>{
      const conflict=conflictById.get(row.conflict_id);if(!conflict)return;
      const conflictId=`conflict:${row.conflict_id}`;
      [...new Set([...(row.side_a_states||[]),...(row.side_b_states||[])])].filter(nation=>nationIds.has(nation)).forEach(nation=>{
        const key=`${nation}\u0000${conflictId}`,existing=participation.get(key),year=Number(row.year);
        if(existing){existing.yearSet.add(year);existing.firstYear=Math.min(existing.firstYear,year);existing.lastYear=Math.max(existing.lastYear,year);}
        else participation.set(key,{source:nation,target:conflictId,kind:'conflict',conflictId:row.conflict_id,yearSet:new Set([year]),firstYear:year,lastYear:year});
      });
    });
    const participationLinks=[...participation.values()].map(link=>({...link,years:link.yearSet.size,yearSet:undefined})).filter(link=>link.years>=state.minYears);
    const linkedConflictIds=new Set(participationLinks.map(link=>link.target));
    const conflictNodes=[...linkedConflictIds].map(id=>{const conflict=conflictById.get(id.slice(9));return {id,label:conflict?.title||id.slice(9),profile:null,conflict,degree:0,weightedDegree:0,entityKind:'conflict'};});
    const nodes=[...nationNodes,...organizationNodes,...conflictNodes];
    const observedLinks=[...edges.values()],membershipLinks=organizationLinks(nodes);
    const basisLinks=state.relationship==='observed'?observedLinks:state.relationship==='organization'?membershipLinks:state.relationship==='participation'?participationLinks:[...participationLinks,...membershipLinks];
    const provisionalDegree=new Map(nodes.map(node=>[node.id,0]));basisLinks.forEach(link=>{provisionalDegree.set(link.source,(provisionalDegree.get(link.source)||0)+1);provisionalDegree.set(link.target,(provisionalDegree.get(link.target)||0)+1);});nodes.forEach(node=>{node.degree=provisionalDegree.get(node.id)||0;});
    let visibleNodes=nodes,visibleBasis=basisLinks;
    if(state.selected&&nodes.some(node=>node.id===state.selected)){
      const adjacency=new Map(nodes.map(node=>[node.id,new Set()]));visibleBasis.forEach(link=>{adjacency.get(link.source)?.add(link.target);adjacency.get(link.target)?.add(link.source);});
      const focus=new Set([state.selected]),queue=[[state.selected,0]];
      while(queue.length){const [id,depth]=queue.shift();if(depth>=2)continue;(adjacency.get(id)||[]).forEach(next=>{if(focus.has(next))return;focus.add(next);queue.push([next,depth+1]);});}
    ...
    state.opponents=new Map(visibleNodes.map(node=>[node.id,new Set(node.profile?normalizedRelations(node.profile,'opposing_states').map(item=>item.country):[])]));
  ...
  const nodeRadius=node=>node.entityKind==='organization'?8+Math.sqrt(node.degree)*1.7:node.entityKind==='conflict'?7+Math.sqrt(node.degree)*1.55:3.5+Math.sqrt(node.degree)*1.3;
  ...
  function nodeColor(node){
    ...
    if(node.entityKind==='conflict')return node.id===state.selected?colors.selected:'#9a5a43';
  ...
  function setSelection(id){
    ...
    const conflictId=String(id||'').startsWith('conflict:')&&conflictById.has(String(id).slice(9));
    state.selected=profileByName.has(id)||organizationId||conflictId?id:'';
    render();
  }
  ...
  function renderInspector(){
    ...
    if(!state.selected){
      ...
      $('#graph-node-type').textContent='Relationship field';$('#graph-node-title').textContent=organizationScope?'Membership network':'Nations, organizations, and conflicts';
      metrics.innerHTML=`<div><span>Displayed nodes</span><strong>${state.nodes.size.toLocaleString()}</strong></div><div><span>Displayed ties</span><strong>${state.links.length.toLocaleString()}</strong></div>`;
      ...
      if(!profile){
        const organizationNode=node.entityKind==='organization',conflictNode=node.entityKind==='conflict';
        $('#graph-node-type').textContent=organizationNode?'Organization':conflictNode?'Conflict':'State';$('#graph-node-title').textContent=node.label;
        metrics.innerHTML=`<div><span>Node type</span><strong>${organizationNode?'Organization':conflictNode?'Conflict record':'Resolved nation'}</strong></div><div><span>Displayed degree</span><strong>${node.degree}</strong></div>`;
        const connected=[...(state.adjacency.get(node.id)||[])].map(id=>state.nodes.get(id)).filter(Boolean).map(item=>({country:item.id,note:item.entityKind||'nation'}));
        detail.innerHTML=`${conflictNode?`<a class="global-nation-link" href="network.html?conflict=${encodeURIComponent(node.conflict.id)}">Open conflict network</a>`:`<p class="boundary-note">${organizationNode?'This organization node connects its sourced member nations. Membership is not treated as alliance, coordination, or causation.':'This node is resolved from the selected source record.'}</p>`}${listBlock('Direct recorded connections',connected,'No direct connections are displayed.')}`;
      }else{
      const displayed=[...(state.adjacency.get(node.id)||[])].map(id=>state.nodes.get(id)).filter(Boolean).map(item=>{const link=state.links.find(edge=>(edge.source===node.id&&edge.target===item.id)||(edge.target===node.id&&edge.source===item.id));return {country:item.id,note:linkDescription(link)};});
      ...
      detail.innerHTML=`<a class="global-nation-link" href="nation.html?country=${encodeURIComponent(node.id)}">Open nation record</a>${listBlock('Direct recorded connections',displayed,'No displayed connection meets this threshold.')}${listBlock('Historic opponents connected through mutual allies',paths,'No two-step mutual-ally path is present at this threshold.')}${listBlock('Historic opponents',opponents,'No opposing state participation is recorded.')}`;
      }
    }
  ...
  function focusNation(id){
    if(!profileByName.has(id)&&!String(id).startsWith('organization:')&&!String(id).startsWith('conflict:'))return;
  ...
  function renderSummary(){
    ...
    const organizationScope=selectedOrganizations().length,scope='nodes',rule=state.relationship==='observed'?'Nation co-participation':state.relationship==='organization'?'Organization membership':state.relationship==='participation'?'Nation-to-conflict participation':'Conflict participation plus selected organization membership',layoutNames={equilibrium:'Balanced field',coalitions:'Connection groups',barabasi:'Hub emphasis','erdos-renyi':'Random comparison','watts-strogatz':'Ring comparison',radial:'Degree rings',lattice:'Comparison grid',prisoner:'Opposing fields','third-party':'Bridge emphasis',pirates:'Influence tiers',temporal:'First-observed time'},model=`${layoutNames[state.topology]||state.topology} · persistent arrangement`;
  ...
  function renderRegister(){
    const selected=state.nodes.get(state.selected),neighbors=selected?[...(state.adjacency.get(selected.id)||[])].map(id=>state.nodes.get(id)).filter(Boolean):[...state.nodes.values()].filter(node=>node.degree).sort(nodePriority).slice(0,30);
    $('#relationship-register-title').textContent=selected?`${selected.label} · ${neighbors.length} direct`:'Highest-reach nodes';
    $('#relationship-register-list').innerHTML=neighbors.map(node=>{
      const link=selected&&state.links.find(edge=>(edge.source===selected.id&&edge.target===node.id)||(edge.target===selected.id&&edge.source===node.id));
      return `<button type="button" data-register-node="${esc(node.id)}" aria-current="${node.id===state.selected}"><time>${esc(node.entityKind||'nation')}</time><strong>${esc(node.label)}</strong><span>${link?esc(linkDescription(link)):`${node.degree} displayed connections`}</span></button>`;
    }).join('')||'<p class="boundary-note">No sourced connections meet the current filters.</p>';
    $('#relationship-register-list').querySelectorAll('[data-register-node]').forEach(button=>button.addEventListener('click',()=>focusNation(button.dataset.registerNode)));
  }
  ...
  function forceLayout(model,spread=1){
    ...
    for(let step=0;step<170;step++){
      ...
      model.links.forEach(link=>{const a=byId.get(link.source),b=byId.get(link.target),dx=b.x-a.x,dy=b.y-a.y,dz=b.z-a.z,d=Math.max(8,Math.hypot(dx,dy,dz)),desired=(link.kind==='organization'?95:link.kind==='conflict'?118:145)+Math.min(80,link.years*2),f=(d-desired)*.012*cooling,fx=dx/d*f,fy=dy/d*f,fz=dz/d*f;a.vx+=fx;a.vy+=fy;a.vz+=fz;b.vx-=fx;b.vy-=fy;b.vz-=fz;});
      nodes.forEach(node=>{const anchor=node.id===centerId ? .16 : state.topology==='equilibrium' ? .025 : .085;node.vx+=(node.homeX-node.x)*anchor;node.vy+=(node.homeY-node.y)*anchor;node.vz+=(node.homeZ-node.z)*anchor;node.vx*=.82;node.vy*=.82;node.vz*=.82;node.x+=node.vx;node.y+=node.vy;node.z+=node.vz;});
    }
  ...
  function renderSVG(model){
    ...
    const edgeElements=model.links.map(link=>{const line=document.createElementNS(ns,'line'),source=byId.get(link.source),target=byId.get(link.target);line.setAttribute('x1',source.x);line.setAttribute('y1',source.y);line.setAttribute('x2',target.x);line.setAttribute('y2',target.y);line.setAttribute('stroke-width',link.kind==='organization'?2.2:link.kind==='conflict'?1.7:.85+Math.log2(1+link.years)*.3);edgeLayer.append(line);const item={link,line};incident.get(link.source).push(item);incident.get(link.target).push(item);return item;});
    const nodeElements=new Map();
    model.nodes.forEach(node=>{const group=document.createElementNS(ns,'g'),title=document.createElementNS(ns,'title'),label=document.createElementNS(ns,'text'),radius=nodeRadius(node),ellipse=document.createElementNS(ns,'ellipse');group.classList.add('global-graph-node');group.dataset.nodeId=node.id;group.dataset.nodeKind=node.entityKind||'nation';group.setAttribute('transform',`translate(${node.x} ${node.y})`);title.textContent=`${node.label} · ${node.entityKind||'nation'} · ${node.degree} displayed connections`;ellipse.setAttribute('rx',node.entityKind==='conflict'?radius:radius*1.25);ellipse.setAttribute('ry',node.entityKind==='conflict'?radius:radius*.78);if(node.entityKind==='organization')ellipse.setAttribute('stroke-dasharray','3 2');group.append(ellipse);const shapes=[ellipse];group.append(title);label.textContent=node.label;label.setAttribute('y',-(radius+5));label.hidden=!node.topLabel;group.append(label);nodeLayer.append(group);nodeElements.set(node.id,{group,shapes,label,node});});
    const scene={svg,viewport,byId,edgeElements,nodeElements,zoom:.92,panX:0,panY:0,yaw:-.18,pitch:.1,drag:null,moved:false,suppressClick:false,animation:null,momentumFrame:null};
    ...
    svg.addEventListener('pointerup',event=>{if(!scene.drag)return;const drag=scene.drag,id=drag.node?.id,moved=scene.moved;scene.drag=null;scene.suppressClick=moved;svg.releasePointerCapture(event.pointerId);if(drag.node&&moved)scene.startMomentum(drag.node,drag.vx,drag.vy);if(!moved){if(id)focusNation(id);else setSelection('');}});
    svg.addEventListener('click',event=>{if(scene.suppressClick){scene.suppressClick=false;return;}if(event.target===svg)setSelection('');});
  ...
  function render(){
    updatePageDescription();const model=buildModel();state.model=model;renderCurrentView(model);renderInspector();renderRegister();renderSummary();
  }
  ...
  document.querySelectorAll('[data-organization]').forEach(input=>input.addEventListener('change',()=>{state.organizations=[...document.querySelectorAll('[data-organization]:checked')].map(item=>item.dataset.organization);state.relationship='combined';$('#graph-relationship').value='combined';state.selected=state.organizations.includes(input.dataset.organization)?`organization:${input.dataset.organization}`:'';render();state.svgScene?.focus(state.selected);}));
  ...
  else if(initialNode.startsWith('conflict:')&&conflictById.has(initialNode.slice(9)))state.selected=initialNode;
  render();
})();
