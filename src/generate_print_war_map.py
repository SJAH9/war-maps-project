...
def add_current_cover(pdf: AtlasPDF, spec: dict, events: list[dict]):
    ...
    pdf.set_xy(16, 132); pdf.set_font("Helvetica", "B", 9); pdf.set_text_color(*YELLOW); pdf.cell(0, 5, f"{spec['window_start'].upper()} - {spec['window_end'].upper()}")
    pdf.set_xy(16, 144); pdf.set_font("Helvetica", "", 9); pdf.set_text_color(205, 205, 192)
    pdf.multi_cell(112, 5.5, f"{len(events)} candidate events connected to their recorded locales. The field remains unresolved beyond the source boundary.")
...
def add_current_principle(pdf: AtlasPDF, spec: dict):
    ...
    pdf.copy(154, 118, 129, f"The source currently begins on {spec['window_start']} and contains events through {spec['window_end']}. The project date is later than the observation boundary; the missing interval remains missing.", 10, INK, 6)
...
def add_current_sources(pdf: AtlasPDF, spec: dict, events: list[dict]):
    ...
    pdf.copy(14, 126, 269, f"{len(events)} unique candidate observations / {spec['window_start']}-{spec['window_end']} / no derived casualty total. Candidate records may overlap. Source snapshots: " + "; ".join(f"{key}: {value}" for key, value in sorted(snapshots.items())) + ". Coding status: " + "; ".join(f"{key}: {value}" for key, value in sorted(statuses.items())) + ".", 8.5, INK, 5)
    pdf.label(14, 153, "Revision path")
    pdf.copy(14, 161, 269, "Regenerate when a new candidate snapshot is deposited. Preserve event IDs, compare revisions, and update the observation boundary. Do not silently carry provisional values to the publication date or convert candidate coding into a final historical record.", 8.8, MUTED, 5.1)
